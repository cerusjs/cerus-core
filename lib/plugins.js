 ...
 * are added to the cerus core. This means that with this class you can expand CerusJS' 
 * functionality.
 * @example
...
class plugins {
	 ...
	 * plugin parameter. The plugin object must contain a name and a version key else it will throw a 
	 * TypeError. If the plugin has already been added it will also throw an Error. If you want to 
	 * re-add a plugin you have to remove the old plugin first. You can also add dependencies to your
	 * plugin. This means that before adding the plugin your list of dependencies is checked. If a 
	 * dependency isn't added yet it'll throw an error. All keys, apart from the name, version and
	 ...
	 * should get access to the cerus object. The rest of the functions are added to the cerus core if 
	 * the key isn't taken yet.
	 * @example
	 * // plugin -> {
	 * //   name: "example",
	 * //   version: "0.0.1",
	 ...
	 * @param {String} plugin.version The version of the plugin.
	...
	add(plugin) {
		...
		this._plugins[name] = {
			name, version, dependencies, funcs: plugin
		};
	}
